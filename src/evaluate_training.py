...
@click.command()
@click.option('--input_path', '-i', help='path to root folder containing data', type=click.Path(exists=True))
...
def main(input_path, output_path, prefix, run_id, model_selection_criterion):
    ...
    # create subdirectory for validation results/ metrics
    res_dir = f'{input_path}/results'
    ...
    for config_folder in [cf for cf in os.listdir(input_path) if cf.startswith(prefix)]:
        for train_folder in glob.glob(f'{input_path}/{config_folder}/train_*_{run_id}/logs/'):
            # extract logs
            n_epochs = int(train_folder.split(os.path.sep)[-3].split('_')[1].replace('epochs', ''))
            train_results = pd.read_csv(f'{train_folder}train_results_epoch{n_epochs}.csv')
            val_results = pd.read_csv(f'{train_folder}val_results_epoch{n_epochs}.csv')
            ...
            # extract best model
            val_results_bm = val_results.iloc[val_results[f'Validation {model_selection_criterion}'].idxmax()]
    ...
    # store best models
    best_models['ID'] = 'max' + best_models['Maximum Sequence Length'].astype(str) \
