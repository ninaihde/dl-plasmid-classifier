...
def prepare_fastq(input_dir, output_dir, prefix, run_id, model_selection_criterion):
    for config_folder in [cf for cf in os.listdir(input_dir) if cf.startswith(prefix)]:
        ...
        for train_folder in glob.glob(f'{input_dir}/{config_folder}/train_*_{run_id}/'):
            ...
            val_results = pd.read_csv(f'{train_folder}logs/val_results_epoch{n_epochs}.csv')
            best_model = val_results.iloc[val_results[f'Validation {model_selection_criterion}'].idxmax()]
            ...
            FN_ids = get_read_ids(merged, 'chr', 'plasmid', output_dir, max_len, cut_method, n_epochs, 'FN')
            FP_ids = get_read_ids(merged, 'plasmid', 'chr', output_dir, max_len, cut_method, n_epochs, 'FP')

            print('Filter and cut sequences...')
            with gzip.open(f'{output_dir}/max{max_len}_cut{cut_method}_{n_epochs}epochs_'
                           f'sequences_FN.fastq.gz', 'wt') as out_file_FN, \
                    gzip.open(f'{output_dir}/max{max_len}_cut{cut_method}_{n_epochs}epochs_'
                              f'sequences_FP.fastq.gz', 'wt') as out_file_FP:
                filter_sequences(input_dir, max_len, out_file_FN, out_file_FP, FN_ids, FP_ids)


def get_read_ids(df, pred_label, gt_label, output_dir, max_len, cut_method, n_epochs, result_type):
    read_ids = df[(df['Predicted Label'] == pred_label) & (df['GT Label'] == gt_label)]['Read ID'].tolist()

    with open(f'{output_dir}/max{max_len}_cut{cut_method}_{n_epochs}epochs_read_ids_{result_type}.txt', 'w') as id_file:
...
def cut_sequence(max_len, record):
    ...
    record.seq = Seq.Seq(record.seq[start_idx:end_idx])
...
@click.option('--output_dir', '-o', help='path to output folder', type=click.Path(exists=True))
...
def main(input_dir, output_dir, prefix, run_id, model_selection_criterion):
    prepare_fastq(input_dir, output_dir, prefix, run_id, model_selection_criterion)

    # for read_file in glob.glob(f'{output_dir}/sequences*.fastq.gz'):
    #    for reference_file in glob.glob(f'{input_dir}/Genomes/*.fasta'):
    #        bam_file = f'{output_dir}/alignment_{os.path.basename(read_file).replace("sequences_", "")}' \
